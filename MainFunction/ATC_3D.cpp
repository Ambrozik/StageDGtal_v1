...
int Meaningful(int argc, char *argv[])
{
    ...
    map<int,double> nbTypeSegment;
    vector<Line3D>  tangentCorverSet=adaptiveTangentCoverDecomposition3D<Point3D>(sequence,vecMT,nbTypeSegment,mode);
    ...
    for(int i = 0 ; i < tangentCorverSet.size(); i++){
        ...
        auto idx = nbTypeSegment.find(tangentCorverSet.at(i).line3D.first.alphaSeg.getThickness());
